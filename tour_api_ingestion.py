...
try:
    logging.info("Fetching country data from API...")
    raw_data = fetch_country_data()  # Fetch raw country data
    country_data = extract_country_data(raw_data)  # Extract structured data from raw data
    df = convert_to_dataframe(country_data)  # Convert structured data to DataFrame
    logging.info(f"DataFrame 'df' has {df.shape[0]} rows and {df.shape[1]} columns.")
    
    # Assuming df_language_expand is defined correctly or remove if not needed
    df_language_expand = pd.DataFrame()  # Placeholder for df_language_expand if you have similar data processing
